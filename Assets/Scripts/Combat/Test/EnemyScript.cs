...
public class EnemyScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        enemyDetection = playerCombatController.GetComponentInChildren<EnemyDetection>();

        //playerCombatController.OnHit.AddListener((x) => OnPlayerHit(x));
        //playerCombatController.OnTrajectory.AddListener((x) => OnPlayerTrajectory(x));
    ...
    public void SetRetreat()
    {
        ...
        IEnumerator PrepRetreat()
        {
            ...
            isMoving = true;
            yield return new WaitUntil(() => Vector3.Distance(transform.position, playerCombatController.transform.position) > 4);
            isRetreating = false;
            StopMoving();
    ...
    void MoveEnemy(Vector3 direction)
    {
        ...
        Vector3 dir = (playerCombatController.transform.position - transform.position).normalized;
        ...
        if(Vector3.Distance(transform.position, playerCombatController.transform.position) < 2)
        {
            StopMoving();
